...
Events dataset consists of `r nrow(df_events)` rows and
`r ncol(df_events)` columns. The `event_date` field ranges from
`r min(df_events$event_date)` to `r max(df_events$event_date)`.
...
-   `quantity`: This field represents the number of units sold in each
    event.
...
-   `net_revenue_usd`: This field represents the net revenue after
    discounts and taxes in USD.
...
It seems that there are `r integrity_issues_count` rows with integrity
issues in the monetary fields.
...
Only `country_latitude` and `country_longitude` are numeric variables in
the `Customers` dataset. We are not going to do correlation analysis for
these fields.
...
There is only one numeric variable in the `Products` dataset:
`base_price_usd_orig`. We will plot its distribution.
