...
dataset <- dataset |>
  mutate(
    ...
    refund_datetime = as.POSIXct(refund_datetime, format = "%d.%m.%Y %H:%M:%S", tz = "UTC"),
...
dataset <- dataset |>
  mutate(
    time_to_refund = as.numeric(difftime(refund_datetime, event_date, units = "days")),
  )
...
Here is the R code for RFM analysis that can be implemented in Power
Query.

First, we load the preprocessed Events table from the previous step to
the `dataset` variable. In Power Query this will be done by referencing
the `Events` table.

```{r load-preprocessed-events-rfm}
...
Here is the R code for ABC/XYZ analysis that can be implemented in Power
Query.

First, we store the preprocessed Events table from the previous step in
the `dataset` variable. In Power Query this will be done by referencing
the `Events` table.
...
Let's visualize the distribution of Coefficient of Variation (CoV) with
thresholds for Y and Z classes.
...
XYZ classes separation looks plausible, so we may proceed adding the
code to Power Query.
